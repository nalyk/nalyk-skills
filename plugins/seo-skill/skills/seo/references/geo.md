...
Check `robots.txt` against the crawler table in
`${CLAUDE_PLUGIN_ROOT}/skills/seo/references/ai-crawlers.md` (single source
of truth, includes allow/block recommendations).
...
Generate `GEO-ANALYSIS.md`. For numeric scores, run the engine
(`cd "${CLAUDE_PLUGIN_ROOT}" && python3 -m engine.cli audit <url>` — AI Search
Readiness category) — NEVER invent a numeric score; prose is for
recommendations only. Include:

1. **Platform breakdown** (Google AIO, ChatGPT, Perplexity — qualitative)
2. **AI Crawler Access Status** (which crawlers allowed/blocked)
3. **llms.txt Status** (present, missing, recommendations)
4. **Brand Mention Analysis** (presence on Wikipedia, Reddit, YouTube, LinkedIn)
5. **Passage-Level Citability** (optimal 134-167 word blocks identified)
6. **Server-Side Rendering Check** (JavaScript dependency analysis)
7. **Top 5 Highest-Impact Changes**
8. **Schema Recommendations** (for AI discoverability)
9. **Content Reformatting Suggestions** (specific passages to rewrite)
