...
AI companies actively crawl the web to train models and power AI search.
For the crawler token table, key distinctions, and example robots.txt rules,
read `${CLAUDE_PLUGIN_ROOT}/skills/seo/references/ai-crawlers.md` (single
source of truth). Cross-reference the `geo` topic for AI visibility strategy.
...
### 6. Core Web Vitals
- Thresholds: read `${CLAUDE_PLUGIN_ROOT}/skills/seo/references/cwv-thresholds.md` (single source of truth)
- INP replaced FID (March 2024) — never reference FID
- Evaluation uses 75th percentile of real user data
- Use PageSpeed Insights API or CrUX data when available
...
(`cd "${CLAUDE_PLUGIN_ROOT}" && python3 -m engine.cli audit <url>`) —
NEVER invent a numeric score; prose is for recommendations only.
